...
class MCTSAgent:
    ...
    def __init__(
        ...
        # 兼容保留：不再作为 rollout 截断条件
        self.rollout_max_depth = rollout_max_depth
        # rollout 采样时使用的 top-k
    ...
    def select_move(self, game_state: GameState) -> Move:
        ...
        try:
            ...
            while rounds < self.num_rounds and time.time() < deadline:
                ...
                rollout_time_sum += (time.time() - sim_start_ts)
                rollout_calls += 1
            ...
            tree_time = max(0.0, think_time - rollout_time_sum)
            avg_rollout_time = (
                rollout_time_sum / rollout_calls if rollout_calls > 0 else 0.0
            )
            print(
                "[MCTS-ROLLOUT] player=%s rollout_time=%.3fs avg_rollout=%.6fs "
                "rollout_calls=%d"
                % (
            ...
            print(
                "[MCTS-TIME] player=%s think_time=%.3fs tree_time=%.3fs rounds=%d "
                "max_seconds=%.3fs"
                % (
                    game_state.next_player.name,
                    think_time,
    ...
    def _simulate(self, game_state):
        """
        Rollout 策略：
        启发式走子，直到棋局真实终局（如连续两手 pass、resign 等）。
        """
    ...
    def _select_rollout_move(self, game_state, legal_moves):
        ...
        # 保留 resign 作为合法终局行为之一。
        if any(m.is_resign for m in legal_moves) and random.random() < 0.002:
            return Move.resign()
