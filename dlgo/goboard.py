"""Core Go board logic: Board, GameState and Move."""
...
class IllegalMoveError(Exception):
    """Raised when trying to place a stone on an occupied point."""
...
class GoString:
    ...
    def merged_with(self, other_string):
        assert other_string.color == self.color
...
class Board:
    ...
    def place_stone(self, player, point):
        ...
        if self._grid.get(point) is not None:
            raise IllegalMoveError(f"Point {point} already occupied")
...
class GameState:
    ...
    def is_over(self):
        ...
        return self._is_same_player_double_pass()

    def _is_same_player_double_pass(self):
        if self.last_move is None or not self.last_move.is_pass:
            return False
        if self.previous_state is None or self.previous_state.previous_state is None:
            return False
        same_player_prev_move = self.previous_state.previous_state.last_move
        return same_player_prev_move is not None and same_player_prev_move.is_pass
    ...
    def winner(self):
        ...
        if self._is_same_player_double_pass():
            return self.next_player
